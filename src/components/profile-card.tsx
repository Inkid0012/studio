          ...
          <div className="aspect-square relative">
            ...
            <div className="absolute bottom-2 left-3 text-white w-full pr-4">
                <div className="flex items-center gap-1.5">
                    <h3 className="font-bold text-base drop-shadow-md">{user.name}</h3>
                    {user.isCertified && (
                        <ShieldCheck className="h-5 w-5 text-green-400 fill-green-900/50" />
                    )}
                </div>
                <div className="flex items-center gap-2 mt-1">
                    <Badge className="bg-cyan-400 text-black border-none text-xs h-5">
                       {user.gender === 'male' ? '♂' : '♀'} {userAge}
                    </Badge>
                     <Badge className="bg-lime-400 text-black border-none text-xs h-5">
                       {distance}km
                    </Badge>
                </div>
            </div>
